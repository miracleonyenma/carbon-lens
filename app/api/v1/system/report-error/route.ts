  ...
  try {
    ...
    const content = `
      <div style="background-color: #fef2f2; border: 1px solid #fee2e2; border-radius: 8px; padding: 16px; margin-bottom: 24px;">
        ...
        <p style="margin-bottom: 8px;"><strong>Message:</strong> <span style="font-family: monospace;">${
          message || "N/A"
        }</span></p>
        <p style="margin-bottom: 8px;"><strong>Digest:</strong> <span style="font-family: monospace;">${
          digest || "N/A"
        }</span></p>
        <p style="margin-bottom: 0;"><strong>Path:</strong> <span style="font-family: monospace;">${
          path || "Unknown path"
        }</span></p>
      </div>
    ...
    await Promise.all(
      admins.map((admin) =>
        emailService.sendEmail({
          subject: `[CRITICAL] Carbon Lens Error Report: ${
            message ? message.substring(0, 50) : digest
          }`,
